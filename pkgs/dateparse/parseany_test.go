...
func TestParseDateTime(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		returnDate, err := ParseDateTime("2009-08-12T22:15:9.99999999Z", "YYYY-MM-DD", "")
		assert.NotEmpty(t, returnDate)
		assert.NoError(t, err)
	})

	t.Run("error_parseAny", func(t *testing.T) {
		_, err := ParseDateTime("2020-20-20", "YYYY-MM-DD", "")
		assert.Error(t, err)
	})
}
