...
func TestMoney_Add(t *testing.T) {
	...
	for _, tc := range tcs {
		m := NewMoney(tc.amount1, EUR)
		om := NewMoney(tc.amount2, EUR)
		r, err := m.Add(om)
...
func TestMoney_Comparison(t *testing.T) {
	pound := NewMoney(100, GBP)
	twoPounds := NewMoney(200, GBP)
	twoEuros := NewMoney(200, EUR)
...
func TestCustomUnmarshal(t *testing.T) {
	...
	UnmarshalJSON = func(m *Money, b []byte) error {
		...
		ref := NewMoney(int64(data["amount"].(float64)), data["currency_code"].(string))
		*m = *ref
		return nil
	}
